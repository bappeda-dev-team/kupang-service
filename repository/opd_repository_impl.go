...
func (repository *OpdRepositoryImpl) Create(ctx context.Context, tx *sql.Tx, opd domain.Opd) (domain.Opd, error) {
	query := "INSERT INTO opd (kode_opd, nama_opd) VALUES ($1, $2) RETURNING id"
	err := tx.QueryRowContext(ctx, query, opd.KodeOpd, opd.NamaOpd).Scan(&opd.Id)
	if err != nil {
...
func (repository *OpdRepositoryImpl) Update(ctx context.Context, tx *sql.Tx, opd domain.Opd) (domain.Opd, error) {
	query := "UPDATE opd SET kode_opd = $1, nama_opd = $2, last_modified_date = NOW() WHERE id = $3"
	_, err := tx.ExecContext(ctx, query, opd.KodeOpd, opd.NamaOpd, opd.Id)
	if err != nil {
		return domain.Opd{}, err
	}
...
func (repository *OpdRepositoryImpl) FindById(ctx context.Context, tx *sql.Tx, id int) (domain.Opd, error) {
	query := "SELECT id, kode_opd, nama_opd FROM opd WHERE id = $1"
	row := tx.QueryRowContext(ctx, query, id)

	var opd domain.Opd
	err := row.Scan(&opd.Id, &opd.KodeOpd, &opd.NamaOpd)
	if err != nil {
...
func (repository *OpdRepositoryImpl) FindAll(ctx context.Context, tx *sql.Tx) ([]domain.Opd, error) {
	query := "SELECT id, kode_opd, nama_opd FROM opd ORDER BY id ASC"
