...
func (repository *OpdRepositoryImpl) Create(ctx context.Context, tx *sql.Tx, opd domain.Opd) (domain.Opd, error) {
	query := "INSERT INTO opd (kode_opd, nama_opd, kode_lembaga) VALUES ($1, $2, $3) RETURNING id"
	err := tx.QueryRowContext(ctx, query, opd.KodeOpd, opd.NamaOpd, opd.KodeLembaga).Scan(&opd.Id)
	if err != nil {
...
func (repository *OpdRepositoryImpl) Update(ctx context.Context, tx *sql.Tx, opd domain.Opd) (domain.Opd, error) {
	query := "UPDATE opd SET kode_opd = $1, nama_opd = $2, kode_lembaga = $3, last_modified_date = NOW() WHERE id = $4"
	_, err := tx.ExecContext(ctx, query, opd.KodeOpd, opd.NamaOpd, opd.KodeLembaga, opd.Id)
	if err != nil {
		return domain.Opd{}, err
	}
...
func (repository *OpdRepositoryImpl) FindById(ctx context.Context, tx *sql.Tx, id int) (domain.Opd, error) {
	query := "SELECT id, kode_opd, nama_opd, kode_lembaga FROM opd WHERE id = $1"
	row := tx.QueryRowContext(ctx, query, id)

	var opd domain.Opd
	err := row.Scan(&opd.Id, &opd.KodeOpd, &opd.NamaOpd, &opd.KodeLembaga)
	if err != nil {
...
func (repository *OpdRepositoryImpl) FindAll(ctx context.Context, tx *sql.Tx) ([]domain.Opd, error) {
	query := "SELECT id, kode_opd, nama_opd, kode_lembaga FROM opd ORDER BY id ASC"
	...
	for rows.Next() {
		var opd domain.Opd
		err := rows.Scan(&opd.Id, &opd.KodeOpd, &opd.NamaOpd, &opd.KodeLembaga)
		if err != nil {
			return []domain.Opd{}, err
		}
